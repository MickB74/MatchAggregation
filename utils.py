...
def generate_load_factor_profile(annual_kwh, hours_per_day, start_hour, year=2024):
    ...
    Scaling_Factor = (WD_sum + WE_sum) / (Weekdays_per_year * WD_sum + Weekends_per_year * WE_sum)
    ...
    Args:
        ...
        hours_per_day (int): Number of operating hours per day (7 inputs for Mon-Sun, 0-24)
    ...
    # Approximate: 52 weeks = 260 weekdays, 104 weekend days
    # More accurate: count actual days
...
def get_market_price_profile_v2(avg_price, return_base_avg=False, year=2024, scale_hist=True):
    ...
    # Map 'Average' to the pre-calculated composite file
    current_dir = os.path.dirname(__file__)
    
    if year == 'Average':
        parquet_file = os.path.join(current_dir, 'ercot_rtm_average.parquet')
    else:
