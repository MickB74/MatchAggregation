...
with tab_offtake:
    ...
    with st.popover("📖 How it Works: CVTA Methodology"):
        ...
        The Buyer pays a monthly **Fixed Capacity Price** ($/MW-mo) to the project developer. This provides the developer with a stable revenue stream to finance the battery.
        * **Calculation:** `Fixed Cost = Capacity (MW) * Fixed Price ($/MW-mo) * 12`

        #### 2. The Floating Leg (Buyer Receives)
        In exchange for the fixed payment, the Buyer receives the **Net Market Revenue** generated by the battery's operation.
        * **Arbitrage Logic**: The model simulates an optimal charging/discharging strategy ("Perfect Foresight") against hourly market prices.
        * **Efficiency**: Accounts for **Round Trip Efficiency (RTE)**—energy is lost during the charge/discharge cycle.
        * **O&M**: Deducts **Variable O&M** ($/MWh) from the gross market revenue.

        #### 3. Net Settlement
        The final "Net Cost" to the Buyer is the difference between the Fixed Payment and the Market Revenue.
        * **Formula**: `Net Settlement = Market Revenue - Fixed capacity Payment`
        * **Adder Price**: Represents the "premium" paid per MWh of battery energy dispatched to the grid.
        """)
